...
def log_activity (msg, log=None):
    ...
    if log is None:
        ...
        log.info("Connecting to autopilot...")


def move_local(device, x_vel, y_vel, z_vel, duration=1, log=None):

    # Move up, down, left, or right relative to the drone's current position.
    log_activity(f"Local move with velocities {x_vel},{y_vel},{z_vel} for {duration} seconds.", log)
...
def arm_device(device, log = None):
